...
// DeesSeek ~ 2025.07.31
// gcc universe_004_batch_01.c -o universe_004_batch_01 -lSDL2 -lm
...
#define STAB_THRESHOLD 1e-7
#define MAX_STABILIZATION_STEPS 100000
#define EQUIVALENCE_THRESHOLD 1e-6
// Physics parameters (carefully tuned)
#define MASS_0 33e-2
#define ATN_COEF 0.95 // 0.995
#define WIDTH 1300
#define HEIGHT 850
#define MAX_PARTICLES 500
#define INITIAL_FLIP_DISTANCE 12.0
#define V_0_INIT 0.001
#define ADD_DIST_MUL 0.27
#define Z_SCALE 1.2
#define Z_SCALE_CONST 0.05
#define G_CONSTANT_ATTRACTIVE 7.0
#define G_CONSTANT_REPULSIVE 5.0
#define INITIAL_TIME_STEP 0.001
#define MAX_DISPLACEMENT_RATIO 5e-3
//chaosregular(x)2025-07-31_17:17:27
...
void initialize_system_with_n_crx(int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        double theta = u * 2.0 * M_PI;
        double phi = acos(2.0 * v - 1.0);
        //double r = radius + (radius * pow((double)rand()/RAND_MAX, 1.0/3.0))/5; // crx
        double r = radius * pow((double)rand()/RAND_MAX, 1.0/3.0); 
        
        particles[i].x = r * sin(phi) * cos(theta);
        particles[i].y = r * sin(phi) * sin(theta);
        particles[i].z = r * cos(phi);
        particles[i].vx = (2.0*(double)rand()/RAND_MAX - 1.0) * V_0_INIT;
        particles[i].vy = (2.0*(double)rand()/RAND_MAX - 1.0) * V_0_INIT;
        particles[i].vz = (2.0*(double)rand()/RAND_MAX - 1.0) * V_0_INIT;
        particles[i].mass = MASS_0;
        particles[i].r = rand() % 256;
        particles[i].g = rand() % 256;
        particles[i].b = 221;
...
double flip_force(double r) {
    ...
    double r2 = r * r;
    double r5 = r2 * r2 * r;
    return G_CONSTANT_ATTRACTIVE / r2 - G_CONSTANT_REPULSIVE / r5;
}
